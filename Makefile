...
FLEX = win_flex
BISON = win_bison
CFLAGS = -std=c99 -Wall -Wextra -g -Isrc/include -Ibuild/generated
LDFLAGS =
...
$(GEN_DIR)/parser.tab.c $(GEN_DIR)/parser.tab.h: src/parser.y src/include/ast.h | $(GEN_DIR)
...
clean:
	powershell -NoProfile -Command "$$paths = @('build','minic.exe','minic','out.c','out.exe'); foreach ($$p in $$paths) { if (Test-Path -LiteralPath $$p) { Remove-Item -LiteralPath $$p -Recurse -Force } }"
